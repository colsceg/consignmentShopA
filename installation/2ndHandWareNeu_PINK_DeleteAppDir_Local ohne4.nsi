...
Section "Uninstall"
  ...
  # Remove AppDir with all data ask user to do or not
  ; Optional section (can be disabled by the user)
	MessageBox MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON2 "Alle vom Benutzer erstellten Daten löschen?" /SD IDNO IDYES true
			GOTO next2
		true:
		MessageBox MB_YESNO|MB_ICONSTOP|MB_DEFBUTTON2 "Alle vom Benutzer erstellten Daten werden gelöscht, weitermachen?" /SD IDNO IDYES true1
			GOTO next2
		true1:
			# Remove AppDir
			;ADD YOUR OWN FILES HERE...
